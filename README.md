...
Веб-платформа поддерживает создание кейсов, проверку Parquet, историю наборов, запуск и отмену анализа. Светлое рабочее место аналитика показывает диаграммы ролей и приоритетов, таблицу всех участников с фильтрами, карточки с потоками и основаниями, направленный граф ближайших переводов и выгрузки результатов. Python SDK дополнительно возвращает операции, временные эпизоды, возобновляемые расследования и досье от трёх локальных аналитических агентов. Просмотр отдельных операций и досье агентов пока не подключён к веб-платформе.
...
4. Запустите анализ. В «Обзоре и участниках» выберите роль или интервал приоритета: фильтры пересекаются, диаграммы показывают весь набор. Найдите точный GID и откройте карточку с потоками и основаниями.
5. Нажмите «Исследовать связи»: направление, сумма и число переводов показаны на графе, профиль выбранного узла — справа. Можно переходить к соседям. Для читаемости отображается до 12 соседей с крупнейшими отдельными связями; полный размер окружения указан над графом. Выгрузки и история находятся ниже результата. Обновите страницу и проверьте сохранённую историю.
6. Для SDK запустите две команды отдельного движка выше; проверьте ссылки на операции в `out-enrichment/dossier.md` и ограничения выводов.
7. Выполните воспроизводимую оценку и восстановление снимка:
