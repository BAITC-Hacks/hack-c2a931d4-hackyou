...
Для будущего интерфейса подготовлены [три изображения-макета](design/README.md): граф с основаниями, досье аналитика и хронология переводов. Они показывают варианты оформления на демонстрационных данных; интеграция интерфейса в платформу не выполнялась.
...
.\.venv\Scripts\python.exe -m tracegraph_ai --input ./data --output ./out --model isolation_forest
```
...
.\.venv\Scripts\python.exe -m tracegraph_ai --input ./data --output ./out-auto --model autoencoder
```
...
```powershell
.\.venv\Scripts\python.exe ml/examples/integrate.py --input ./data --output ./out-sdk
.\.venv\Scripts\python.exe ml/examples/integrate.py --load ./out-sdk --output ./out-restored
```
...
```powershell
.\.venv\Scripts\python.exe ml/examples/enrich_investigation.py --analysis ./out --output ./out-enrichment
```
...
.\.venv\Scripts\python.exe ml/examples/integrate.py --load ./out --output ./out-restored
.\.venv\Scripts\python.exe ml/examples/evaluate_enrichment.py --analysis ./out --output ./out-enrichment-eval
```

В Linux используйте `python` из активированной `.venv` вместо Windows-пути. Проверки охватывают точность ID и сумм, сохранность операций, временные эпизоды, восстановление снимка, продолжение ветки и основания агентных выводов. Пример `integrate.py --load` проверяет работу с сохранённым результатом без обучения.

После успешного полного анализа в `out/` должны присутствовать все девять файлов из таблицы выше. Досье из команды обогащения находится в `out-enrichment/dossier.md`, исходный структурированный ответ — в `enrichment.json`. Оценка обогащения пишет `out-enrichment-eval/enrichment_metrics.json` и `enrichment_metrics.md`. Готовые измерения и условия проверки: [метрики обогащения](docs/enrichment-metrics.md), [проверка базового движка](docs/validation.md).
...
Форматы и интерпретация результатов агентных расчётов описаны в [документации обогащения](docs/investigation-enrichment.md#проверки-и-измерения).
