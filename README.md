...
Локальное рабочее место аналитика для кейса HackAlem «Граф денег» и объяснимый AI/ML-движок **TraceGraph AI 0.3.0**.
...
Полный анализ создаёт 3 CSV, 5 JSON bundles и `manifest.json`. Обогащение создаёт `enrichment.json` и читаемое `dossier.md`. Снимки 0.2.0 и 0.3.0 загружаются без повторного обучения. В Linux замените путь интерпретатора на `.venv-engine/bin/python`.
...
В этой версии проверены установленный wheel, полный CLI и восстановление снимков; **31 тест движка прошёл**, полный CPU Autoencoder-анализ занял **24,02 с** на проверяемой машине. Обогащение прошло **8 контрольных синтетических сценариев**; на четырёх выбранных узлах исходного графа проверены **10/10 маршрутов и 533/533 ссылки**. Условия, знаменатели и воспроизведение: [метрики обогащения](docs/enrichment-metrics.md), [проверка движка](docs/validation.md), [исходный JSON измерений](docs/metrics/enrichment-0.3.0.json).
